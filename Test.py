...
# dibujar la caja del cuadro delimitador del contorno
    ...
    for (x, y) in box:
        cv2.circle(orig, (int(x), int(y)), 5, (0, 255, 255), -1)
...
# seguido por el punto central entre las coordenadas inferiores
    (x1, x2, br, bl) = box
    print("Coordenadas" + str(box))
    print("x1 :" + str(x1))
    (tltrX, tltrY) = midpoint(x1, x2)  # Punto medio Eje x
    (blbrX, blbrY) = midpoint(bl, br)  # Punto medio Eje y
# Calcular el punto central entre superior-inferior izquierdos
# seguido de superior-inferior derechos
    (tlblX, tlblY) = midpoint(x1, bl)
    (trbrX, trbrY) = midpoint(x2, br)
...
# calcular la distancia Euclídea entre los puntos medios
    print(" "+str(tltrX)+"  "+str(tltrY)+"  " +
          str(blbrX)+"  "+str(blbrY))

    print(" "+str(tlblX)+"  "+str(tlblY)+"  " +
          str(trbrX)+"  "+str(trbrY))

    # Altura  desde el punto medio
    dA = dist.euclidean((tltrX, tltrY), (blbrX, blbrY))
    # Ancho desde el punto medio
